...
class ScreenViewCluster(u.WidgetWrap):
    ...
    def update(self):
        ...
        cpu_text = f"CPU:    [{cpu_bar}] {overall['cpus_alloc']}/{overall['cpus_total']} cores ({overall['cpu_util']:.1f}%)"
        widgets.append(u.Text(cpu_text))
        ...
        mem_text = f"Memory: [{mem_bar}] {mem_alloc}/{mem_total} ({overall['mem_util']:.1f}%)"
        widgets.append(u.Text(mem_text))

        # === GPU resources ===
        if gpu_stats:
            widgets.append(u.Divider())
            widgets.append(u.AttrMap(u.Text("GPU Resources"), 'jobheader'))
            widgets.append(u.Divider("─"))
        ...
        if gpu_nodes:
            widgets.append(u.Divider())
            widgets.append(u.AttrMap(u.Text("GPU Nodes"), 'jobheader'))
            widgets.append(u.Divider("─"))
            ...
            for node in gpu_nodes:
                for gpu in node.gpus:
                    ...
                    if gpu.used > 0 and gpu.indices != "N/A":
                        widgets.append(u.Text(f"                              └─ Using: GPU {gpu.indices}"))

        # === All nodes summary ===
        widgets.append(u.Divider())
        widgets.append(u.AttrMap(u.Text("All Nodes"), 'jobheader'))
        widgets.append(u.Divider("─"))

        # Group nodes by state
        nodes_by_state = cluster.get_nodes_by_state()
        for state in sorted(nodes_by_state.keys()):
            node_list = nodes_by_state[state]
            node_names = ", ".join(sorted([n.name for n in node_list]))
            widgets.append(u.Text(f"{state:15s} ({len(node_list):2d}): {node_names}"))
